...
class BondWidgetsMixin(BaseMixin):
    ...
    def get_bond_widgets(
        ...
        def manual_bond_update(b):
            ...
            self.update_struct(updated_graph)
            self._plot_struct()
        
        update_button.on_click(manual_bond_update)
        
        bond_acc = widgets.Accordion(
            children=[widgets.HBox([bond_box, update_button])], titles=("bond length", )
        )
        bonding = widgets.Dropdown(options=["Auto", "Manual"], description="bonding")       
        algos = [
        ...
        bonding_algos.observe(self._update_bonds, names="value")
        
        stack = widgets.Stack([bonding_algos, bond_acc], selected_index=0)
        widgets.jslink((bonding, "index"), (stack, "selected_index"))
        return widgets.VBox([bonding, stack])
    
    def _update_bonds(self, change: dict) -> None:
        """
