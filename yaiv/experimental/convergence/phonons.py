...
def phonons_vs_cutoff(folder,savefig=None):
    ...
    for i in range(0,len(data),2):
        color=cmap(int(i/2))
        plot_data=data[i+1]
        plt.plot(plot_data[:,0],plot_data[:,1],'.-',color=color,label=data[i])
        plt.plot(plot_data[:,0],plot_data[:,2:],'.-',color=color)
    plt.ylabel("Freqs in $\Gamma$ (cm-1)")
